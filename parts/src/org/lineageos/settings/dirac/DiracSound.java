...
public class DiracSound extends AudioEffect {
    ...
    private static final int DIRACSOUND_PARAM_HEADSET_LIST = 19;
    ...
    private int getIntParameter(int parameter) {
        ...
        checkStatus(count);
        if (count != 1) {
            throw new IllegalStateException("Incomplete MiSound scalar: " + count);
        }
        return value[0];
    }
    ...
    public int[] getHeadsetList() {
        ...
        int size = getParameter(DIRACSOUND_PARAM_HEADSET_LIST, reply);
        checkStatus(size);
        ...
        if (count < 0 || count > (size - 4) / 4) {
            // Some legacy alioth effects expose an incomplete five-byte reply.
            return new int[0];
        }
    ...
    public void setMusic(int enable) {
        if (enable != 0 && enable != 1) {
            throw new IllegalArgumentException("Invalid music mode");
        }
    ...
    public void setHeadsetType(int type) {
        if (type < 0 || type > 255) {
            throw new IllegalArgumentException("Invalid headset type");
        }
    ...
    public void setLevel(int band, float level) {
        if (band < 0 || band >= EQ_BAND_COUNT) {
            throw new IllegalArgumentException("Invalid EQ band");
        }
        if (!Float.isFinite(level)) {
            throw new IllegalArgumentException("Non-finite EQ level");
        }
